...
- **Topic inference**: Auto-capture and topicless manual saves use local `flan-t5-small` to choose a durable topic label, falling back to `general`
...
- `/memory topics on` / `/memory topics off` — enable/disable local topic inference
